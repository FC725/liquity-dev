...
const supportedNetworks = ["homestead", "kovan", "rinkeby", "ropsten", "goerli", "fantom"];
...
export const LiquityProvider: React.FC<LiquityProviderProps> = ({
  ...
  const connection = useMemo(() => {
    ...
    if (config && provider && account && chainId) {
      ...
      try {
        console.log("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@");
        console.log("GetSigner = ",provider.getSigner(account));

        console.log("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@");
         let _chainId = _connectByChainId(provider, provider.getSigner(account), chainId, {
          userAddress: account,
          frontendTag: config.frontendTag,
          useStore: "blockPolled"
        });
        console.log("Let Chain Id = ", _chainId);
        return _chainId;
      } catch {}
    }
